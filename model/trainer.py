...
class DGITrainer(Trainer):
    ...
    def predict(self, batch):
        ...
        print('in predict user_one shape: {}'.format(User_One.shape))
        Item_feature = torch.index_select(self.item_hidden_out, 0, self.model.item_index) # item_num * hidden_dim
        print('the equality between Item_feature and self.item_hidden_out: {}'.format(torch.equal(Item_feature, self.item_hidden_out)))
